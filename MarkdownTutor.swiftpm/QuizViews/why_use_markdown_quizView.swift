...
struct why_use_markdown_quizView:View {
    ...
    var body:some View{
        VStack{
            ScrollView{
                Multiple_Choice_View(question: question_1).frame(minWidth: UIScreen.main.bounds.width/2,minHeight: 400)
                Divider()
                Multiple_Choice_View(question: question_2).frame(minWidth: UIScreen.main.bounds.width/2,minHeight: 400)
            }
            Spacer()
            Button(action: {
